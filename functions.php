<?php
/*
 *  WP Bootstrap Sass Custom functions, support, custom post types and more.
 *  Author: tone4hook
...
function wpbootstrapsass_header_scripts()
{
    if ($GLOBALS['pagenow'] != 'wp-login.php' && !is_admin()) {
        ...
        wp_register_script('bootstrap-js', get_template_directory_uri() . '/js/bootstrap.min.js', array('jquery'), '3.3.7');
        wp_enqueue_script('bootstrap-js'); // Enqueue it!

        wp_register_script('wpbootstrapsassscripts', get_template_directory_uri() . '/js/scripts.js', array(), '1.0.0'); // Custom scripts
        wp_enqueue_script('wpbootstrapsassscripts'); // Enqueue it!
    }
}
...
function wpbootstrapsass_styles()
{
    ...
    wp_register_style('wpbootstrapsass', get_template_directory_uri() . '/style.css', array(), '1.0', 'all');
...
if (function_exists('register_sidebar'))
{
    ...
    register_sidebar(array(
        ...
        'id' => 'widget-area-1',
        'before_widget' => '<div id="%1$s" class="%2$s well">',
        'after_widget' => '</div>',
        'before_title' => '<h3>',
        'after_title' => '</h3>'
    ));
...
function wpbootstrapsass_blank_view_article($more)
{
    global $post;
    return '... <p><a class="view-article btn btn-default" href="' . get_permalink($post->ID) . '" role="button">' . __('Read more', 'wpbootstrapsass') . ' <span class="glyphicon glyphicon-arrow-right" aria-hidden="true"></span></a></p>';
}
...
add_filter('excerpt_more', 'wpbootstrapsass_blank_view_article'); // Add 'View Article' button instead of [...] for Excerpts
